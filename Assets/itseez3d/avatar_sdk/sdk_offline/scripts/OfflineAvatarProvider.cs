...
{
	/// <summary>
	/// Implementation of the IAvatarProvider for offline version of the Avatar SDK.
	/// </summary>
	public class OfflineAvatarProvider : IAvatarProvider
	{
		#region Data
		// SDK session.
		private Session session = null;

		private HashSet<PipelineType> supportedCalculationPipelines = new HashSet<PipelineType> { PipelineType.FACE };

		private OfflineResourceManager resourceManager = new OfflineResourceManager();
		#endregion

		#region Ctor

		public OfflineAvatarProvider()
		{
			session = new Session();
		}

		public OfflineAvatarProvider(Session session)
		{
		...
		/// <param name="name">Currently not used by offline SDK</param>
		/// <param name="description">Currently not used by offline SDK</param>
		...
		{
			if (!supportedCalculationPipelines.Contains(pipeline))
				throw new Exception(string.Format("{0} is not supported by Offline SDK!", pipeline));

			if (avatarResources == null)
				avatarResources = resourceManager.GetResources(AvatarResourcesSubset.DEFAULT);

			var request = new AsyncRequest<string>(AvatarSdkMgr.Str(Strings.GeneratingAvatar));
			AvatarSdkMgr.SpawnCoroutine(InitializeAvatarFunc(photoBytes, avatarResources, request));
			return request;
		}
		...
		/// <summary>
		/// Do nothing. For offline SDK all data is already stored locally.
		/// </summary>
		...
		public AsyncRequest<byte[]> GetHaircutPreviewAsync(string avatarCode, string haircutId)
		{
			...
			AvatarSdkMgr.SpawnCoroutine(GetHaircutPreviewFunc(haircutId, request));
			return request;
		}
...
#region private methods
		...
		/// </summary>
		private IEnumerator InitializeAvatarFunc(byte[] photoBytes, AvatarResources avatarResources, AsyncRequest<string> request)
		{
			...
			var initializeAvatar = session.InitializeAvatarAsync(rawPhoto, resources: avatarResources);
		...
		private IEnumerator StartAndAwaitAvatarCalculationFunc(string avatarCode, AsyncRequest request)
		{
			...
			yield return request.AwaitSubrequest(session.CalculateAvatarOfflineAsync(avatarCode), 1.0f);
		...
		private IEnumerator GetHaircutsIdFunc(string avatarCode, AsyncRequest<string[]> request)
		{
			...
			request.Result = haircutsWithFilenames.Keys.ToArray();
			request.IsDone = true;
		...
		private IEnumerator GetHaircutMeshFunc(string avatarCode, string haircutId, AsyncRequest<TexturedMesh> request)
		{
			...
			{
				var extractHaircutRequest = session.ExtractHaircutAsync(CoreTools.ConvertHaircutIdToNewFormat(haircutId));
		...
		/// </summary>
		private IEnumerator GetHaircutPreviewFunc(string haircutId, AsyncRequest<byte[]> request)
		{
