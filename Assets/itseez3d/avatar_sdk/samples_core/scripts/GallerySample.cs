...
namespace ItSeez3D.AvatarSdkSamples.Core
{
    public static class CurrentAvatar
    {
        ...
        public static GalleryAvatarState state;
	...
	public abstract class GallerySample : MonoBehaviour, IAvatarClickHandler
	{
        ...
        public IAvatarProvider m_AvatarProvider
        {
            get
            {
		...
		protected IEnumerator Await(AsyncRequest r, string avatarCode)
		{
			while (!r.IsDone)
			{
				...
				if (string.IsNullOrEmpty(avatarCode))
				{
					...
					// _progressText = string.Join("  -->  ", progress.ToArray());
				}
				else
				{
        ...
        public void UpdateProgressText(string _text)
        {
            _progressText = _text;
        }
		...
		public IEnumerator CreateNewAvatar(byte[] photoBytes)
		{
			...
			if (initializeAvatar.IsError)
			{
                ...
                CurrentAvatar.state = GalleryAvatarState.FAILED;
                yield break;
			}
            ...
            CurrentAvatar.avatarCode = avatarCode;
            CurrentAvatar.state = GalleryAvatarState.COMPLETED;
		}
