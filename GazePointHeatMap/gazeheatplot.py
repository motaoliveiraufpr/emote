# code by Tobias Roeddiger
# modified by Claudemir Casa
...
#required args
parser.add_argument('-i', '--input-path', type=str, default='..\ExportedData', required=False, help='path to the csv input')
parser.add_argument('-wd', '--width-display', type=int, default='1024', required=False, help='an integer representing the display width')
parser.add_argument('-hd', '--height-display', type=int, default='768', required=False, help='an integer representing the display height')
...
display_width = args['width_display']
display_height = args['height_display']
alpha = args['alpha']
...
#get a list of filtered files
folders_list = os.listdir(input_path)

for folder in folders_list:
    formatted_list = []
    formatted_files = []
    _files = input_path + '\\' + folder + '\\answers\\eyetracking\\gaze_data'
    files_list = os.listdir(_files)
    for ifile in files_list:
        if re.match('.*_formatted.csv', ifile):
            formatted_list.append(_files + '\\' + ifile)
            formatted_files.append(ifile)
    
    i = 0
    for file_path in formatted_list:
        _background_image = ''
        _output_image = ''

        if not background_image:
            striped = re.sub('_formatted.csv', '', formatted_files[i])
            _background_image = _files + '\\' + striped + '.png'
            _output_image = _files + '\\' + striped + '_heatmap'
        
        img_file = Image.open(_background_image)
        w, h = img_file.size
        ...
        with open(file_path) as f:
            ...
            if (len(raw) > 0):
                ...
                print('generating heatmap of file: {0}'.format(_background_image))
                draw_heatmap(gaze_data, (w, h), alpha=alpha, savefilename=_output_image, imagefile=_background_image, gaussianwh=ngaussian, gaussiansd=sd)
            else:
                print('file {0} has incomplete data on database!'.format(_background_image))
